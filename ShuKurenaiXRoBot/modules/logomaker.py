...
from ShuKurenaiXRoBot import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("./ShuKurenaiXRoBot/resources/fonts/*")
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [ShuKurenaiXRoBot](https://t.me/ShuKurenaiXRoBot)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./ShuKurenaiXRoBot/resources/fonts"):
            os.remove(font_)
...
async def logo_(event):
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [bullyxguardianbot](https://t.me/bullyxguardianbot)",
            force_document=False,
        )
        os.remove(flnme)
        await xx.delete()
